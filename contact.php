    ...
    <script>
      ...
      if (!onay){
        window.alert("Onay verilmedi!");
          ...
          window.alert("Telefon boş");
          return false;
      }
      else if (cinsiyet!=="kadin" && cinsiyet!=="erkek"){
        window.alert("Cinsiyet seçilmedi!");
        return false;
      }
      else if (nokta<1){
    ...
    <div class="container mx-auto">
      ...
      <div id="app" class="max-w-xs mx-auto py-2">
        <form method="POST" class="flex flex-col justify-self-center items-center space-y-2" name="form" action="contact_process.php" onsubmit="return kontrol()" >
          ...
          <input value="erkek" v-model="cinsiyet" required type="radio" id="erkek" name="cinsiyet">
          <label for="kadin">Kadın</label>
          <input value="kadin" v-model="cinsiyet" type="radio" id="kadin" name="cinsiyet">
          </div> 
      ...
      setup(){
        ...
        const etIsaret=email.value.split("@").length-1;
          ...
          if (!onay.value) {
            ...
            window.alert("Telefon boş");
          } else if (cinsiyet.value !== "kadin" && cinsiyet.value !== "erkek") {
            window.alert("Cinsiyet seçilmedi!");
          } else if (nokta < 1) {
            window.alert("Email'de . yok !");
          } else if (etIsaret!==1){
